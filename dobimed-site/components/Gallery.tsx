...
export default function Gallery() {
    ...
    return (
        <section className="py-20 bg-slate-950 text-white select-none border-t border-slate-900" id="gallery">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                    {images.map((img, index) => (
                        <motion.div
                            ...
                            className="relative h-48 md:h-64 bg-slate-900 rounded-xl overflow-hidden cursor-pointer group shadow-lg border border-slate-800"
                    ...
                    >
                        <div className="absolute inset-0 z-[60] pointer-events-none flex flex-col justify-between p-4">
                            <div className="flex justify-end">
                                <button onClick={closeLightbox} className="pointer-events-auto p-2 bg-white/10 hover:bg-white/30 rounded-full text-white transition">
                                    <X size={32}/>
                                </button>
                            </div>
                            <div className="flex justify-between items-center flex-1 px-2">
                                <button onClick={showPrev} className="pointer-events-auto p-3 bg-white/10 hover:bg-white/30 rounded-full text-white transition hover:scale-110">
                                    <ChevronLeft size={40}/>
                                </button>
                                <button onClick={showNext} className="pointer-events-auto p-3 bg-white/10 hover:bg-white/30 rounded-full text-white transition hover:scale-110">
                        ...
                        <div className="relative z-50 w-full h-full p-4 md:p-10 flex items-center justify-center overflow-hidden" onClick={(e) => e.stopPropagation()}>
                            <motion.img
                                key={selectedIndex}
                                src={currentImage.src}
                                alt={currentImage.alt}
                                initial={{scale: 0.9, opacity: 0}}
                                animate={{ scale: isZoomed ? 2.5 : 1, opacity: 1, x: 0, y: 0 }}
